// todo.spec.js
// ============================================================
// Playwright E2E tests for the Todo Electron App
//
// KEY CHANGE: The app is launched ONCE via beforeAll() and
// closed ONCE via afterAll(). All test cases share the same
// running instance. Between tests, beforeEach() resets state
// by clearing all todos so each test starts clean.
// ============================================================
...
test.beforeAll(async () => {
  app = await electron.launch({
    args: [path.join(__dirname, '../../todo-electron/main.js')],
  });
...
test.beforeEach(async () => {
  // Delete all existing todos by clicking every Delete button until none remain
  while (true) {
  ...
  // Make sure we are on the "All" filter
...
test('TC-05: Empty input does not add a task', async () => {
  // Click add without any input text
  await window.click('#add-btn');
  await window.waitForTimeout(150);
...
test('TC-08: Completed task has strikethrough text', async () => {
  ...
  const textStyle = await window.$eval('.todo-item.completed .todo-text', el =>
    window.getComputedStyle(el).textDecoration
  );
  // text-decoration should contain 'line-through'
  expect(textStyle).toContain('line-through');
});
...
test('TC-15: Filter "Active" shows only incomplete tasks', async () => {
  ...
  const checkboxes = await window.$$('.todo-checkbox');
  await checkboxes[1].click(); // complete the second
  await window.waitForTimeout(150);
